...
def get_data(filters):
    ...
    if filters.get("from_date") and filters.get("to_date"):
        ...
        today = getdate(nowdate())  # ensure it's a date
    ...
    for restaurant in restaurants:
        ...
        # Scheduled audits
        scheduled_audits = frappe.get_all(
            "Scheduled Audit Visit",
            filters={
                "restaurant": restaurant.name,
                "auditor": ["in", auditors],
        ...
        # Submissions
        audit_submissions = frappe.get_all(
            "Audit Submission",
            filters={
                "restaurant": restaurant.name,
                "auditor": ["in", auditors],
...
def get_summary(data):
    ...
    avg_compliance = sum([d["compliance_rate"] for d in data]) / total_restaurants if total_restaurants else 0

    return [
        {"label": _("Total Restaurants"), "value": total_restaurants, "indicator": "blue"},
        {"label": _("Critical Status"), "value": critical_restaurants, "indicator": "red"},
        {"label": _("Good Status"), "value": good_restaurants, "indicator": "green"},
        {"label": _("Total Overdue Audits"), "value": total_overdue, "indicator": "red"},
        {"label": _("Total Missed Daily"), "value": total_missed_daily, "indicator": "orange"},
        {
...
def get_charts(data):
    ...
    restaurant_names = [
        d["restaurant_name"][:15] + "..." if len(d["restaurant_name"]) > 15 else d["restaurant_name"]
        for d in data
    ]
    ...
    return {
        "data": {
            ...
            "datasets": [
                {"name": _("Compliance Rate"), "values": compliance_rates}
            ]
        },
        "type": "bar",
        "colors": ["#1f77b4"],
